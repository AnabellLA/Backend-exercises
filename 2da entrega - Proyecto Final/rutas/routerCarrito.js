...
const { Carrito } = require("../DAOs/carrito.daos.class.js");
...
routerCarrito.get("/:id/productos", async (req, res, next) => {
    try {
            const carrito = await claseCarrito.getById(req.params.id);
            res.status(200).json(carrito?.productos ?? { error: "El carrito no fue encontrado" });
    } catch (error) {
        next(error);
    }
});
